...
class CovidChartGenerator:
    ...
    def generate_animated_map(self, df=None, scope="world", metric="cum_cases", chart_type="choropleth",
                              log_scale=False, fit_dates=True, plotly_template=None):
        """
        ...
        if scope.lower() not in self.scopes:
            logger.warning("Selected scope not supported ({}). \nPlease select from: {}.".format(scope, ", ".join(
                self.scopes)))
            return

        if metric.lower() not in self.metrics.keys():
            logger.warning("Selected metric not supported ({}). \nPlease select from: {}.".format(metric, ", ".join(
                self.metrics.keys())))
            return
        ...
        if chart_type == "choropleth":
            ...
            fig.update_layout(coloraxis_colorbar=dict(title=self.metrics[metric].capitalize(),
                                                      yanchor="middle", y=0.5, ticks="outside"))
    ...
    def generate_animated_bar_chart(self, df=None, scope="world", x="cum_cases", y="default", x_cutoff=None, top_n=None,
        ...
        if x.lower() not in self.metrics:
            logger.warning("Selected x not supported ({}). \nPlease select from: {}.".format(x, ", ".join(
                self.metrics.keys())))
            return

        if y.lower() not in list(self.metrics.keys()) + ["default"]:
            logger.warning("Selected y not supported ({}). \nPlease select from: {}.".format(y, ", ".join(
    ...
    def generate_animated_scatter_plot(self, df=None, x="cum_cases", y="mortality_rate", size="population_2018",
        ...
        if size.lower() not in list(self.metrics.keys()) + ["population_2018"]:
            logger.warning("Selected size metric not supported ({}). \nPlease select from: {}.".format(size, ", ".join(
